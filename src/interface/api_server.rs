...
pub async fn create_server() -> std::io::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            .route("/api/doc", web::get().to(|r| handlers::handle_statics_by_path("doc/swaggerui/index.html".to_string(), r)))
            .route("/api/risk-api.yaml", web::get().to(|r| handlers::handle_statics_by_path("doc/risk-api.yaml".to_string(), r)),)
            .route(
                "/api/{filename:.*}",
                web::get()
